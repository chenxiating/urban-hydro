...
def main(nodes_num = int(100), process_core_name = None):
	...
	print('Node number in the network is:', nodes_num)
	rain_nodes = G.nodes
...
# Simulations
	for network in range(4):
		...
		G = hn.create_networks(g_type = 'gn', nodes_num = nodes_num, level = init_level, diam = 1, node_area = 500, outlet_level = outlet_level, outlet_node_area = outlet_node_area)
...
if __name__ == '__main__':
	main()
	#main(nodes_num = int(100), process_core_name = None)
=======
def main(nodes_num = int(10), process_core_name = None):
    ## Assign Network Properties ##
    # In this step we build the network based on different criteria
    np.random.seed(seed = 1358)
    #G = pickle.load(open('graph_10nodes', 'rb'))
