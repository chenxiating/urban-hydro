...
def main(nodes_num = int(100), process_core_name = None, antecedent_soil_moisture = 0.1, mean_rainfall_inch = 1, hours = 8, dt_str = None, 
kernel=None, soil_nodes_range = [0, 100, 100],plot_iter=False,dt=0.001,seed=None):
    ...
    dt = dt
    simulation_timesteps = round(hours/dt)
    ...
    for network in range(1):
        ...
        datafile_name = 'dataset_'+str(round(mean_rainfall_inch,1))+'-inch_'+str(nodes_num)+'-nodes_'+str(hours)+'-day_'+dt_str+'soil_moisture-'+str(round(antecedent_soil_moisture,1))+'_'+str(process_core_name)+'_'+str(soil_nodes_range[1])+'.pickle'
        ...
        for soil_nodes in output_df['soil_nodes_list']:
            ...
            for i in range(0,simulation_timesteps):
                ...
                if outlet_flow_rate>max_outlet_flow_rate:
                    max_outlet_flow_rate=outlet_flow_rate           
                if H.nodes[0]['level'] > max_outlet_water_level:
                    max_outlet_water_level = H.nodes[0]['level']
                # flood_nodes_this_round = [k for k in H.nodes if ((k > 0) & (h_new[k]>= upstream_diam[k]))]
                flood_nodes_this_round = [k for k in rain_nodes if (h_new[k]>= upstream_diam[k])&(k>0)]
                ...
                flood_nodes = flood_nodes + sum(h_new[k]>= upstream_diam[k] for k in H.nodes if k != 0)
            ...
            # print('edge_h',nx.get_edge_attributes(H,'edge_h'))
            # print('node_level',nx.get_node_attributes(H,'level'))
            # print({n: str(round(H.nodes[n].get('level') + H.nodes[n].get('elev') ,1)) for n in H.nodes})
            # hn.draw_network_timestamp(gph = H, soil_nodes = soil_nodes, edge_attribute = 'edge_dq', label_on = True, flood_level = flood_level)
            # print([data for u, v, data in H.in_edges(0, data = 'diam')])
            ...
            total_overflow = sum(H.nodes[k]['overflow'] for k in H.nodes)
...
def four_subplots(hours, simulation_timesteps, depth, disp_df, outlet_level):
    """
    this plot function is created for debugging.
    """
    x = np.linspace(0,hours,num=simulation_timesteps)
...
# def plotstuff(gph, x, depth, dispersion, outlet_level, hours = hours, simulation_timesteps = simulation_timesteps):
#     try: 
#         x
#         print(x)
#     except NameError:
